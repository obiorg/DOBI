...
public class SupervisionService {
    ...
    public List<MachineStateDto> getAllMachineStates() {
        return machineManagerService.getActiveCollectors().values().stream()
                .map(collector -> new MachineStateDto(
                collector.getMachineId(),
                collector.getMachineName(),
                collector.getCurrentStatus(),
                collector.getTagsReadCount(),
                collector.getConnectedSince(),
                collector.getConnectionCount()
        ))
                .collect(Collectors.toList());
    }
}
