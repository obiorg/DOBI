...
import org.dobi.moka7.Exceptions.S7Exception;
...
public class S7Client {
    ...
    /**
     * A Connect method that generate exception with Error
     *
     * @return
     */
    private void TCPDoConnect() throws S7Exception {
        SocketAddress sockaddr = new InetSocketAddress(IPAddress, ISOTCP);
        LastError = 0;
        try {
            TCPSocket = new Socket();
            ...
            throw new S7Exception(errTCPConnectionFailed, e);
//            LastError = errTCPConnectionFailed;
        }
    }
    ...
    public void DoConnect() throws S7Exception {
        LastError = 0;
        if (!Connected) {
            TCPDoConnect();
            if (LastError == 0) // First stage : TCP Connection
...
//        return LastError;
    ...
    public void DoConnectTo(String Address, int Rack, int Slot) throws S7Exception {
        int RemoteTSAP = (ConnType << 8) + (Rack * 0x20) + Slot;
        SetConnectionParams(Address, 0x0100, RemoteTSAP);
        DoConnect();
    }
